"""
FlapDash - A Flappy Bird-style Game

A fun pygame-based game where the player controls a randomly-colored bird
and navigates it through pipes while avoiding obstacles. Features progressive
difficulty, high score tracking, and sound effects.
...
# Define asset paths relative to the src directory
ASSETS_DIR = '../assets'
IMAGES_DIR = f'{ASSETS_DIR}/images'
SOUNDS_DIR = f'{ASSETS_DIR}/sounds'

bg = pygame.image.load(f'{IMAGES_DIR}/bg.png').convert_alpha()
ground_img = pygame.image.load(f'{IMAGES_DIR}/ground.png').convert_alpha()
button1_img = pygame.image.load(f'{IMAGES_DIR}/restart1.png').convert_alpha()
button2_img = pygame.image.load(f'{IMAGES_DIR}/exit.png').convert_alpha()
button3_img = pygame.image.load(f'{IMAGES_DIR}/play.png').convert_alpha()


# function to draw text with an outline
def draw_text_with_outline(text, font, text_col, outline_col, x, y):
    """
    Renders text with an outlined effect on the screen.
    
    Args:
        text (str): The text to render
        font (pygame.font.Font): The font to use for rendering
        text_col (tuple): RGB color tuple for the main text
        outline_col (tuple): RGB color tuple for the outline
        x (int): X-coordinate for text positioning
        y (int): Y-coordinate for text positioning
    """
...
def show_game_title():
    """
    Displays the 'FlapDash' game title with a dark red color and black outline
    on the center-top portion of the screen.
    """
    dark_red = (200, 0, 0) 
    black = (0, 0, 0) 
...
def draw_text(text, font, text_col, x, y):
    """
    Renders simple text without outline on the screen.
...
def reset_game():
    ...
    Returns:
        int: The final score (which becomes 0)
    
    Resets:
        - All pipes are cleared
        - Player position returns to starting location
        - Score resets to 0
        - Pipe frequency and scroll speed return to initial values
    """
...
class Ibon(pygame.sprite.Sprite):
    ...
    Features:
    - Randomly selected color variant (blue, green, red, or yellow)
    - Animated wings with 4-frame animation cycle
    - Physics-based gravity and jumping mechanics
    - Collision detection with obstacles
    - Sound effects for jumping
    
    Attributes:
        images (list): The selected bird animation frames
        vel (float): Current vertical velocity
        clicked (bool): Tracks if space key is held to prevent continuous input
    """
    def __init__(self, x, y):
        ...
        for num in range(1, 5):
            img = pygame.image.load(f'{IMAGES_DIR}/blue_bird{num}.png')
            self.blue_bird_images.append(img)
        
        for num in range(1, 5):
            img = pygame.image.load(f'{IMAGES_DIR}/green_bird{num}.png')
            self.green_bird_images.append(img)

        for num in range(1, 5):
            img = pygame.image.load(f'{IMAGES_DIR}/red_bird{num}.png')
            self.red_bird_images.append(img)

        for num in range(1, 5):
            img = pygame.image.load(f'{IMAGES_DIR}/yellow_bird{num}.png')
        ...
        self.vel = 0
        self.clicked = False
        self.flying_sound = pygame.mixer.Sound(f'{SOUNDS_DIR}/flying_sound.mp3')
...
class Pipe(pygame.sprite.Sprite):
    """
    Represents an obstacle pipe that scrolls across the screen.
    
    Pipes are created in pairs (top and bottom) with a gap in between for the
    player to navigate through.
    
    Args:
        x (int): Initial X-coordinate
        y (int): Base Y-coordinate (center point)
        position (int): 1 for top pipe, -1 for bottom pipe
    
    Attributes:
        pipe_gap (int): The vertical gap between top and bottom pipes
    """
    def __init__(self, x, y, position):  
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load(f'{IMAGES_DIR}/pipe.jpg')
        self.rect = self.image.get_rect()
...
class Button():
    ...
    Buttons are used for game menus and controls (Play, Restart, Exit).
    Detects mouse collisions and click events.
    
    Args:
        x (int): Center X-coordinate
        y (int): Center Y-coordinate
        image (pygame.Surface): The button image to display
    """
...
DATA_DIR = '../data'
score_file = f'{DATA_DIR}/score.text'
    ...
    if len(pipe_group) > 0:
        ...
        if pass_pipe == True:
              if ibon_group.sprites()[0].rect.left > pipe_group.sprites()[0].rect.right:
                    score += 1
                    pass_pipe = False
                    score_sound = pygame.mixer.Sound(f'{SOUNDS_DIR}/score_sound.mp3')
...
# handle collision and game over conditions
    died_sound = pygame.mixer.Sound(f'{SOUNDS_DIR}/died_sound.mp3')
